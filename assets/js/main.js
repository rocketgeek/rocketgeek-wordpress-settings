(function( $, document ) {
	...
	var rgs = {
		...
		cache: function() {
			...
			rgs.els.tab_links = $('.rgs-nav__item-link');
			rgs.els.submit_button = $( '.rgs-button-submit' );
		},
		...
		on_ready: function() {
			...
			// on ready stuff here
			rgs.cache();
			rgs.trigger_dynamic_fields();
			rgs.setup_groups();
			rgs.tabs.watch();
			rgs.watch_submit();
			rgs.control_groups();
			rgs.setup_visual_radio_checkbox_field();
			rgs.importer.init();

			$( document.body ).on( 
				'change',
				'input, select, textarea, .rgs-visual-field input[type="radio"], .rgs-visual-field input[type="checkbox"]', 
				rgs.control_groups
			);
		},
		...
		trigger_dynamic_fields: function() {
			...
			rgs.setup_timepickers();
			rgs.setup_datepickers();
		...
		tabs: {
			 ...
			 */
			watch: function() {
				var tab_id = rgs.tabs.get_tab_id();

				if ( tab_id ) {
					rgs.tabs.set_active_tab( tab_id );
				}

				rgs.els.tab_links.on( 'click', function( e ) {
					// Show tab
				...
				$( '.rgs-internal-link' ).click( rgs.tabs.follow_link );
			},
			...
			set_tab_id: function( tab_id ) {
				if ( !rgs.tabs.has_storage ) {
					return;
				}

				localStorage.setItem( rgs.tabs.get_option_page() + '_rgs_tab_id', tab_id );
			},
			...
			get_tab_id: function() {
				...
				if ( window.location.hash ) {
					...
					if ( $( `.rgs-nav a[href="${window.location.hash}"]` ).length ) {
				...
				return localStorage.getItem( rgs.tabs.get_option_page() + '_rgs_tab_id' );
			},
			...
			set_active_tab: function( tab_id ) {
				var $tab = $( tab_id ),
					$tab_link = $( '.rgs-nav__item-link[href="' + tab_id + '"]' );

				if ( $tab.length <= 0 || $tab_link.length <= 0 ) {
					// Reset to first available tab.
					$tab_link = $( '.rgs-nav__item-link' ).first();
					tab_id = $tab_link.attr( 'href' );
					$tab = $( tab_id );
				}

				// Set tab link active class
				rgs.els.tab_links.parent().removeClass( 'rgs-nav__item--active' );
				$( 'a[href="' + tab_id + '"]' ).parent().addClass( 'rgs-nav__item--active' );

				// Show tab
				$( '.rgs-tab' ).removeClass( 'rgs-tab--active' );
				$tab.addClass( 'rgs-tab--active' );

				rgs.tabs.set_tab_id( tab_id );
			},
		...
		setup_groups: function() {
			...
			$( document ).on( 'click', '.rgs-group__row-add', function() {

				var $group = $( this ).closest( '.rgs-group' ),
					$row = $( this ).closest( '.rgs-group__row' ),
					template_name = $( this ).data( 'template' ),
					$template = $( $( '#' + template_name ).html() );

				$template.find( '.rgs-group__row-id' ).val( rgs.generate_random_id() );

				$row.after( $template );

				rgs.reindex_group( $group );
			...
			$( document ).on( 'click', '.rgs-group__row-remove', function() {

				var $group = jQuery( this ).closest( '.rgs-group' ),
					$row = jQuery( this ).closest( '.rgs-group__row' );
		...
		reindex_groups: function() {
			...
			$groups.each( function( index, group ) {
				rgs.reindex_group( jQuery( group ) );
			} );
		},
		...
		reindex_group: function( $group ) {
			...
			if ( 1 === $group.find( ".rgs-group__row" ).length ) {
				$group.find( ".rgs-group__row-remove" ).hide();
			} else {
				$group.find( ".rgs-group__row-remove" ).show();
			}

			$group.find( ".rgs-group__row" ).each( function( index ) {
				...
				$( this ).find( '.rgs-group__row-index span' ).html( index );
		...
		watch_submit: function() {
			rgs.els.submit_button.on( 'click', function() {
				var $button = $( this ),
					$wrapper = $button.closest( '.rgs-settings' ),
		...
		control_groups: function() {
			...
			$( '.show-if' ).each( function( index ) {
				...
				// Field.
				if ( 'td' === parent_tag || 'label' === parent_tag || rgs.is_visual_field( element ) ) {
					element.closest( 'tr' ).hide();

					rgs.maybe_show_element( element, function() {
				...
				// Section.
				if ( 'div' === parent_tag && ! rgs.is_visual_field( element ) ) {
					element.prev().hide();
					element.next().hide();
					if ( element.next().hasClass( 'rgs-section-description' ) ) {
			...
			$( '.hide-if' ).each( function( index ) {
				...
				if ( 'td' === parent_tag || 'label' === parent_tag || rgs.is_visual_field( element ) ) {
					...
					rgs.maybe_hide_element( element, function() {
						element.closest( 'tr' ).hide();
					} );
				}
		...
		is_visual_field: function( element ) {
			return element.parent().hasClass( 'rgs-visual-field__item-footer' );
		},
			...
			Array.from( controllers ).forEach( function( control_group ) {
				...
				if ( item.includes( '&&' ) ) {
					...
					Array.from( and_group ).forEach( function( and_item ) {
						if ( ! rgs.get_show_item_bool( show_item, and_item ) ) {
							show_item = false;
						}
					});
					...
					var show_item = true;
					show_item = rgs.get_show_item_bool( show_item, item );
		...
		maybe_hide_element: function( element, callback ) {
			...
			Array.from( controllers ).forEach( function( control_group ) {
				...
				if ( item.includes( '&&' ) ) {
					...
					var hide_item = true;
					Array.from( and_group ).forEach( function( and_item ) {
						if ( ! rgs.get_show_item_bool( hide_item, and_item ) ) {
							hide_item = false;
						}
					});
					...
					var hide_item = true;
					hide_item = rgs.get_show_item_bool( hide_item, item );
		...
		get_show_item_bool: function( show = true, item ) {
			...
			var control_value = rgs.get_controller_value( control, values );
		...
		setup_visual_radio_checkbox_field: function() {
			var checked_class = 'rgs-visual-field__item--checked';

			$( document ).on( 'change', '.rgs-visual-field input[type="radio"], .rgs-visual-field input[type="checkbox"]', function() {
				var $this = $( this ),
					$list = $this.closest( '.rgs-visual-field' ),
					$list_item = $this.closest( '.rgs-visual-field__item' ),
					$checked = $list.find( '.' + checked_class ),
					is_multi_select = $list.hasClass( 'rgs-visual-field--image-checkboxes' );
		...
		importer: {
			init: function () {
				...
				$( '.rgs-import__button' ).click( function () {
					$( this ).parent().find( '.rgs-import__file_field' ).trigger( 'click' );
				} );

				$( ".rgs-import__file_field" ).change( function ( e ) {
					$this = $( this );
					$td = $this.closest( 'td' );

					var file_field = $this.get( 0 ),
						settings = "",
						rgs_import_nonce = $td.find( '.rgs_import_nonce' ).val();
						rgs_import_option_group = $td.find( '.rgs_import_option_group' ).val();
					
					
					if ( 'undefined' === typeof file_field.files[ 0 ] ) {
						alert( rgs_vars.select_file );
						return;
					}
					...
					rgs.importer.read_file_text( file_field.files[ 0 ], function ( content ) {
						try {
							JSON.parse( content );
							settings = content;
						} catch {
							settings = false;
							alert( rgs_vars.invalid_file );
						}
						...
						$.ajax( {
							...
							type: 'POST',
							data: {
								action: 'rgs_import_settings',
								settings: settings,
								option_group: rgs_import_option_group,
								_wpnonce: rgs_import_nonce
							},
							success: function ( response ) {
								if ( response.success ) {
									location.reload();
								} else {
									alert( rgs_vars.something_went_wrong );
								}
